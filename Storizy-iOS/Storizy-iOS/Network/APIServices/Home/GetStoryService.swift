...
struct GetStoryService {
    static let shared = GetStoryService()
    
    func getStory(accessToken: String, userId: Int, completionHandler: @escaping (ResponseCode, Any) -> (Void)) {

        let url = APIUrls.getStoryURL + "\(userId)/story"
        print("url\(url)")
